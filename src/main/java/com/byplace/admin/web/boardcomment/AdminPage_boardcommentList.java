...
@WebServlet("/adminPage_boardcommentList")
public class AdminPage_boardcommentList extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public AdminPage_boardcommentList() {
        super();
    }
...
//		if(session.getAttribute("USER")!=null && ((UserDTO)session.getAttribute("USER")).getUser_type().equals("관리자")) {
			...
			int recordCount = dao.count(searchColumn, searchValue, board_no);
			request.setAttribute("recordCount", recordCount);
			request.setAttribute("pageSize", pageConfigure.pageSize);
			
			Cookie cookie[] = request.getCookies();
			String sort = "boardcomment_no asc";
			String column="", column_sort=""; 
			if(cookie != null) {
				for(int i=0;i<cookie.length;i++) {
					if(cookie[i].getName().equals("boardcommentListColumn")) {
						column = cookie[i].getValue(); 
					}
					if(cookie[i].getName().equals("boardcommentListColumn_sort")) {
			...
			request.getRequestDispatcher("./adminPage_boardcommentList.jsp").forward(request, response);
//		} else {
