...
@WebServlet("/adminPage_loginList")
public class AdminPage_loginList extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public AdminPage_loginList() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(session.getAttribute("USER")!=null && ((UserDTO)session.getAttribute("USER")).getUser_type().equals("관리자")) {
			...
			int recordCount = dao.countLoginUser(searchColumn, searchValue);
			...
			if(cookie != null) {
				for(int i=0;i<cookie.length;i++) {
					if(cookie[i].getName().equals("loginListColumn")) {
						column = cookie[i].getValue(); 
					}
					if(cookie[i].getName().equals("loginListColumn_sort")) {
			...
			request.getRequestDispatcher("./adminPage_loginList.jsp").forward(request, response);
		} else {
