...
@WebServlet("/adminPage_loginList_JSON")
public class AdminPage_loginList_JSON extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public AdminPage_loginList_JSON() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(session.getAttribute("USER") != null && ((UserDTO)session.getAttribute("USER")).getUser_type().equals("관리자")) {
			...
			if(st.hasMoreTokens()) {
				Cookie cookie = new Cookie("loginListColumn", st.nextToken());
				response.addCookie(cookie);
			}
			if(st.hasMoreTokens()) {
				Cookie cookie = new Cookie("loginListColumn_sort", st.nextToken());
				response.addCookie(cookie);
			}
			
			AdminUserDAO userDAO = new AdminUserDAO();
			List<UserDTO> loginList = userDAO.findLoginList(cmd, searchColumn, searchValue, currentPage, pageSize);
