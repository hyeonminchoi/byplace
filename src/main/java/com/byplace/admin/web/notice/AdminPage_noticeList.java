...
@WebServlet("/adminPage_noticeList")
public class AdminPage_noticeList extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public AdminPage_noticeList() {
        super();
    }
...
//		if(session.getAttribute("USER")!=null && ((UserDTO)session.getAttribute("USER")).getUser_type().equals("관리자")) {
			...
			String sort = "notice_no asc";
			String column="", column_sort=""; 
			if(cookie != null) {
				for(int i=0;i<cookie.length;i++) {
					if(cookie[i].getName().equals("noticeListColumn")) {
						column = cookie[i].getValue(); 
					}
					if(cookie[i].getName().equals("noticeListColumn_sort")) {
			...
			request.getRequestDispatcher("./adminPage_noticeList.jsp").forward(request, response);
//		} else {
