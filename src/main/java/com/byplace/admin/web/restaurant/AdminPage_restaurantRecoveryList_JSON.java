...
@WebServlet("/adminPage_restaurantRecoveryList_JSON")
public class AdminPage_restaurantRecoveryList_JSON extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public AdminPage_restaurantRecoveryList_JSON() {
        super();
    }
...
//		if(session.getAttribute("USER") != null && ((UserDTO)session.getAttribute("USER")).getUser_type().equals("관리자")) {
			...
			if(st.hasMoreTokens()) {
				Cookie cookie = new Cookie("restaurantRecoveryListColumn", st.nextToken());
				response.addCookie(cookie);
			}
			if(st.hasMoreTokens()) {
				Cookie cookie = new Cookie("restaurantRecoveryListColumn_sort", st.nextToken());
				response.addCookie(cookie);
			}
			
			AdminRestaurantDAO restaurantDAO = new AdminRestaurantDAO();
			List<RestaurantDTO> restaurantList = restaurantDAO.findByRecoveryList(cmd, searchColumn, searchValue, currentPage, pageSize);
