...
@WebServlet("/adminPage_reviewList")
public class AdminPage_reviewList extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public AdminPage_reviewList() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(session.getAttribute("USER")!=null && ((UserDTO)session.getAttribute("USER")).getUser_type().equals("관리자")) {
			...
			int recordCount = dao.count(searchColumn, searchValue, restaurant_no);
			request.setAttribute("recordCount", recordCount);
			request.setAttribute("pageSize", pageConfigure.pageSize);
			
			Cookie cookie[] = request.getCookies();
			String sort = "review_no asc";
			String column="", column_sort=""; 
			if(cookie != null) {
				for(int i=0;i<cookie.length;i++) {
					if(cookie[i].getName().equals("reviewListColumn")) {
						column = cookie[i].getValue(); 
					}
					if(cookie[i].getName().equals("reviewListColumn_sort")) {
			...
			request.getRequestDispatcher("./adminPage_reviewList.jsp").forward(request, response);
		} else {
