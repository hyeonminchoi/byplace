...
@WebServlet("/adminPage_userBlackList")
public class AdminPage_userBlackList extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public AdminPage_userBlackList() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(session.getAttribute("USER")!=null && ((UserDTO)session.getAttribute("USER")).getUser_type().equals("관리자")) {
			...
			int recordCount = dao.countBlack(searchColumn, searchValue);
			...
			if(cookie != null) {
				for(int i=0;i<cookie.length;i++) {
					if(cookie[i].getName().equals("userBlackListColumn")) {
						column = cookie[i].getValue(); 
					}
					if(cookie[i].getName().equals("userBlackListColumn_sort")) {
			...
			request.getRequestDispatcher("./adminPage_userBlackList.jsp").forward(request, response);
		} else {
