...
public class AdminPage_userWithdrawalList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(session.getAttribute("USER")!=null && ((UserDTO)session.getAttribute("USER")).getUser_type().equals("관리자")) {
			...
			int recordCount = dao.countWithdrawal(searchColumn, searchValue);
			...
			if(cookie != null) {
				for(int i=0;i<cookie.length;i++) {
					if(cookie[i].getName().equals("userWithdrawalListColumn")) {
						column = cookie[i].getValue(); 
					}
					if(cookie[i].getName().equals("userWithdrawalListColumn_sort")) {
