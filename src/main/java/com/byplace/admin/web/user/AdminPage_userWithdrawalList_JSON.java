...
public class AdminPage_userWithdrawalList_JSON extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(session.getAttribute("USER") != null && ((UserDTO)session.getAttribute("USER")).getUser_type().equals("관리자")) {
			...
			if(st.hasMoreTokens()) {
				Cookie cookie = new Cookie("userWithdrawalListColumn", st.nextToken());
				response.addCookie(cookie);
			}
			if(st.hasMoreTokens()) {
				Cookie cookie = new Cookie("userWithdrawalListColumn_sort", st.nextToken());
				response.addCookie(cookie);
			}
			
			AdminUserDAO userDAO = new AdminUserDAO();
			List<UserDTO> userList = userDAO.findByWithdrawalList(cmd, searchColumn, searchValue, currentPage, pageSize);
