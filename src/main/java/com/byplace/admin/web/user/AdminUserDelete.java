...
//		if(session.getAttribute("USER") != null && ((UserDTO)session.getAttribute("USER")).getUser_type().equals("관리자")) {
		long user_no = Long.parseLong(request.getParameter("user_no"));
			...
			if(result==1) { //탈퇴 성공
				writer.println("<script>alert('" + "탈퇴에 성공했습니다" + "'); window.location.href = document.referrer;</script>");
			} else { //탈퇴 실패
				writer.println("<script>alert('" + "탈퇴에 실패했습니다" + "'); window.location.href = document.referrer;</script>");
			}
//		} else {
