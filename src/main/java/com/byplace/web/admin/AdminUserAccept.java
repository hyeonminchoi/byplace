...
//		if(session.getAttribute("USER") != null && ((UserDTO)session.getAttribute("USER")).getUser_type().equals("관리자")) {
			...
			if(result==1) { //추가 성공
				writer.println("<script>alert('" + "권한 승인에 성공했습니다" + "'); location.href='./adminPage_userList';</script>");
			} else { //추가 실패
				writer.println("<script>alert('" + "권한 승인에 실패했습니다" + "'); location.href='./adminPage_userList';</script>");
			}
//		} else {
//			response.sendRedirect("./index.jsp");
//		}
