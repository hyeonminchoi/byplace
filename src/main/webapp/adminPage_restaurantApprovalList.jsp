...
function search(){
	...
	location.href="./adminPage_restaurantApprovalList?" + urlSearch;
}
function changed(){
	...
	$.ajax({
		url: "./adminPage_restaurantApprovalList_JSON",
		type: "GET",
		dataType: "json",
...
<script>
	...
	function accept(no){
		if(confirm("음식점 등록을 승인하겠습니까?")){
			location.href='./adminRestaurantAccept?restaurant_no=' + no;
		} else{
			location.href='./adminPage_restaurantApprovalList';
		}
	}
	
	function refuse(no){
		if(confirm("음식점 등록을 거부하겠습니까?")){
			location.href='./adminRestaurantRefuse?restaurant_no=' + no;
		} else{
			location.href='./adminPage_restaurantApprovalList';
		}
	}
