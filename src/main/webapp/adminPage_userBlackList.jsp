...
function search(){
	...
	location.href="./adminPage_userBlackList?" + urlSearch;
}
function changed(){
	...
	$.ajax({
		url: "./adminPage_userBlackList_JSON",
		type: "GET",
		dataType: "json",
		...
		success: function(userList){
			...
			for(var i in userList){
				...
				temp += "	<td>" + "<button type=\"button\" onclick=\'release(" + userList[i].user_no + ")\'>해제</button>" + "</td>";
				temp += "</tr>";
			}
...
<script>
	function release(no){
		if(confirm("블랙을 해제 하겠습니까?")){
			location.href='./adminUserRelease?user_no=' + no;
		} else{
			location.href='./adminPage_userBlackList';
		}
	}
